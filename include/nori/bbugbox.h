 ...
 * \code
 * TBoundingBugBox<Vector3i> integerBBox(Point3i(0, 1, 3), Point3i(4, 5, 6));
 * TBoundingBugBox<Vector2d> doubleBBox(Point2d(0.0, 1.0), Point2d(4.0, 5.0));
 * \endcode
 *
...
struct TBoundingBugBox : TBoundingStructure<_PointType>
{
    enum {
    ...
    TBoundingBugBox(const PointType &p) 
        : min(p), max(p) { }
    /// Create a bounding box from two positions
    TBoundingBugBox(const PointType &min, const PointType &max)
        : min(min), max(max) {
    }

    template <typename PointType>
    TBoundingBugBox &operator=(const TBoundingBugBox<PointType> &bbox){
        this->min = bbox.min;
        this->max = bbox.max;
        return *this;
    }
    ...
    bool operator==(const TBoundingBugBox &bbox) const {
     ...
     *               box boundary should be excluded in the test
     */
    bool contains(const TBoundingBugBox &bbox, bool strict = false) const {
        if (strict) {
    ...
    bool contains(BS &_bbox, bool strict = false) const {
        TBoundingBugBox &bbox = dynamic_cast<TBoundingBugBox &>(_bbox);
     ...
     */
    bool overlaps(const TBoundingBugBox &bbox, bool strict = false) const {
        if (strict) {
     ...
     */
    Scalar squaredDistanceTo(const TBoundingBugBox &bbox) const {
        Scalar result = 0;
     ...
     * the axis-aligned bounding box and \c bbox.
     */
    Scalar distanceTo(const TBoundingBugBox &bbox) const {
    ...
    void clip(const TBoundingBugBox &bbox) {
    ...
    void expandBy(const TBoundingBugBox &bbox) {
    ...
    static TBoundingBugBox merge(const TBoundingBugBox &bbox1, const TBoundingBugBox &bbox2) {
        return TBoundingBugBox(
            bbox1.min.cwiseMin(bbox2.min),
            bbox1.max.cwiseMax(bbox2.max)
        );
    }
    // BS* merge(BS &_bbox1, BS &_bbox2) {
    //     TBoundingBugBox &bbox1 = dynamic_cast<TBoundingBugBox &>(_bbox1);
    //     TBoundingBugBox &bbox2 = dynamic_cast<TBoundingBugBox &>(_bbox2);
    //     return merge(bbox1, bbox2);
    // }

    /// Merge bounding box list
    static TBoundingBugBox merge(const std::vector<TBoundingBugBox> &bbox_list) {
        PointType merged_min, merged_max;
        ...
        for(size_t i=0;i<bbox_list.size();++i){
            merged_min.cwiseMin(bbox_list[i].min);
            merged_max.cwiseMax(bbox_list[i].max);
        }
        return TBoundingBugBox(merged_min, merged_max);
    }

    /// Return the index of the largest axis, same with getMajorAxis()
    ...
    bool rayIntersect(const Ray3f &ray) const {
        ...
        for (int i=0; i<3; ++i) {
            ...
            if (ray.d[i] == 0) {
                if (origin < minVal || origin > maxVal)  // i-th axis speed is 0, no offset, no intersection
                    return false;
            } else {
                ...
                nearT = std::max(t1, nearT); // maximum of minimum
                farT = std::min(t2, farT); // minimum of maximum
